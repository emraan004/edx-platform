...
def template_assets_path(instance, filename):
    """
    Custom path for credentials templates file assets.
    ...
    name = os.path.join(
        'credential_certificate_template_assets',
        str(instance.id),
        filename
    )
...
def signatory_assets_path(instance, filename):
    """
    Custom path for signatories file assets.

    Arguments:
        instance: Signatory object
        filename: file to upload
...
class UserCredentialAttribute(TimeStampedModel):
    ...
    """
    user_credential = models.ForeignKey(UserCredential)
